  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.a
          id="whatsapp-btn"
          href="https://wa.me/917893422232"
          target="_blank"
          rel="noopener noreferrer"
          initial={{ opacity: 0, scale: 0, y: 50 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0, y: 50 }}
          whileHover={{ scale: 1.1, rotate: 5, boxShadow: '0 0 30px rgba(34,197,94,0.6)' }}
          whileTap={{ scale: 0.9 }}
          className="fixed bottom-8 right-8 z-[100] w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center text-white shadow-2xl shadow-green-900/40 cursor-pointer group"
          aria-label="Contact on WhatsApp"
        >
          {/* Pulsing glow */}
          <span className="absolute inset-0 rounded-full bg-[#25D366] opacity-30 animate-ping group-hover:animate-none group-hover:opacity-0 transition-all duration-300" />
